...
async function getData(
  apiUrl: string,
  dataType: string
): Promise<Category[] | Work[]> {
  const response = await fetch(apiUrl + dataType);
  return await response.json();
}

const getDataSet = async function (
  dataTypes: Array<"works" | "categories">
): Promise<DataSet> {
...
const startEditingHandler = async function () {
  ...
  async function addWorkHandler(event: Event) {
    ...
    const selectCategoryInput = document.getElementById(
      "add-work-category"
    ) as HTMLInputElement;
    ...
    function selectFileHandler(event: Event) {
      ...
      const fileInput = fileInputId
        ? (document.getElementById(fileInputId) as HTMLInputElement)
        : undefined;
